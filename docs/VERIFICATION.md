...
| C9 | 消费组安全性：Stable 良构（全员就绪/每分区恰一 owner/owner ∈ 成员集）、同代分配唯一；阴性对照（部分就绪 Sync）反例已检出——CompletingSync 入口"全员完成加入"守卫为必要设计。收敛性归仿真层/T-Q.4 | L2 | PlusCal/TLC `spec/ConsumerGroup.tla` | ✅ v0.1 + v0.2（fencing/超时踢除）；活性二分闭合（2026-09-10）：四级公平性（无/WF 粗粒度/WF 收口+Sync/WF 全部子动作）均红，根因=模型允许无限成员重报 churn、收口永非持续可用——收敛性需环境假设（churn 有界，v0.3 加预算旋钮可模型层闭合），现实收敛性归 T-Q.4；L3 待 coordinator 完整实现 | `make -C spec consumer-group` / `-demo` |
...
| C14 | **规约 v0.2 扩展**（不变式评审清单）：① crash 模型加 synced 边界（与 §3 故障模型对齐，机器检查"commit ⇒ 多数派已持久"）；② 消费组 CommitOffsets fencing（带 generation 的提交动作 + 僵尸消费）；③ 超时踢除路径；④ 活性公平性实验（已闭合为二分结论：需 churn 有界环境假设，见 CONSUMERGROUP-LIVENESS-WIP.md）；⑤ view 回退方向实验。已识别的结构性恒真不变式（InvGenAssignmentUnique 等）标注降级，防回归价值保留 | L2 | TLC | ⬜ 规划（VERIFICATION-GUIDE 未建模边界清单） | — |
