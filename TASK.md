...
| T-M3.3 | 🟨 | KIP-848 新消费组协议 | **设计 ✅（2026-09-17 [ADR-19](docs/adr-19-kip848-consumer-groups.md) 草案，待评审）**：ConsumerGroupHeartbeat 单循环、服务端 Range 分配、member-epoch fence、组类型分叉；块 a 状态机+Range 分配器 ✅（consumer_group_tests ×4：java 对拍/fence/幂等续租/退订）**；待落地：协议面（b）/e2e（c）/ConsumerGroup.tla 扩展+阴性对照（d）。原待办： 新旧协议混布 rebalance 收敛；客户端（kafka-clients 4.x）跑通 | T-M1.1 | [Kafka §5](../docs/01-apache-kafka.md) |
