...
| T-M3.6 | 🟨 | share groups（KIP-932） | **评估完成（2026-09-18，[纪要](docs/research/2026-09-18-share-groups-evaluation.md)）**：触发条件达成——三客户端支持度实测全就绪（librdkafka 2.15 SHARE_ACKNOWLEDGE 符号面 / kafka-clients 3.9.1 ShareConsumer 早鸟 / franz-go v1.21.6 `ShareGroup`+`Record.Ack()`）；协议面精读（76 与 848 同构、78/79 区间化 ack、coordinator RPC 族不对外宣告）；落地面 = 848 基建复用 + 分区侧区间化 ack 状态机（Morax INT8RANGE[] 对照收敛）；规模 ≈ 4-5 会话，spike 先行（franz-go 验收主面）。**spike ✅（2026-09-19，GO）**：share_group.rs 内存状态机（成员/epoch fence/轮转分配/区间化交付面：acquire-accept-release-reject/30s record lock/交付上限 5）+ 76/78/79 三 handler + 会话注册面（KIP-227 增量语义，spike 实证①）+ fetch 无 member-epoch 校验（实证②）；run_share_spike.sh franz-go 四验收面 PASS。**v1 转正面 ✅（2026-09-19 同日）**：块 c share-state 持久化（data_dir/share-state/{group}.json，ack 变更即落盘；acquired 瞬态=重启即全员解锁）+ 遗忘分区注册删除 + 长轮询（复用分区 actor pending 机制，MaxWaitMs）+ e2e [5] 重启后 accepted 不重投。**多成员 rebalance + REJECT 归档过滤 ✅（2026-09-19）**：assignment_epoch 代次追踪（成员集变化即全员重下发）+ 轮转重算 + serve 端 assigned_partitions 过滤（会话注册面 ≠ 分配面）+ 归档批次剔除（REJECT-cursor 连续性）；e2e [5] 多成员分配/[6] REJECT 后不重投。**待做（块 d）**：多成员 failover 迁移/长轮询唤醒精细化 | T-M3.3 ✅ | KIP-932 |
| T-M3.3.1 | ✅ | 848 生命周期硬化 | **[研究对照](docs/research/2026-09-18-reference-comparison.md) 行动清单全量落地（2026-09-18）**：P0 session timeout（成员滑动 timer + deadline sweep，崩溃分区泄漏封口）+ 撤销确认闭环（owned 跟踪 + pending_release 门控，双 owner 消除）；P1 差分下发（未变→Assignment=null + full-request 判定）+ previous-epoch 容错/epoch-0 rejoin（应答丢失恢复）；P2 Describe 三值状态（含 Reconciling）/interval 节流/group max size（81）/SubscribedTopicRegex 同步解析（names∪resolved）/静态成员（-2 paused + **InstanceId 顶替 rejoin**，净人数豁免组容量）/rebalance timeout（**撤销宽限到期 fence 僵尸 owner**，非仅放行分区）；coordinator 测试 8→16；**franz-go 848 e2e 全链零回归** | T-M3.3 ✅ | kafka trunk 对照 |
| T-M4.3.1 | ✅ | 分层存储 v1.1 | **行动清单全量落地（2026-09-18）**：段 key 编入 leader epoch（记录追加式——failover/截断漂移不再覆盖）；上传异步化（OffloadJob + 专用 OS 线程 offloader + 字节配额，移出 produce 关键路径）；运行期孤儿 GC（mtime 宽限窗）；读穿透 range 化（记录带 crc32c + 稀疏批边界索引）；**duramen S3 适配器 ✅（arrow-rs 0.14 薄层 + 实机联调全链 PASS：produce→上传→读穿透→重启恢复 400/400；联调修复 duramen Last-Modified 头 RFC 2822——修复与后续在 duramen 仓认领：`duramen/TASK.md` S3 互操作合规面 + WIRE-CONTRACT 日期契约）** | T-M4.3 ✅ | slatedb/duramen 对照 |
## M4 生产化

| ID | 状态 | 任务 | 产出物 | 验收标准 | 依赖 | 参考 |
|---|---|---|---|---|---|---|
| T-M4.1 | 🟨 | 安全 | **一期 ✅（2026-09-18，[ADR-22](docs/adr-22-security-admin-plane.md)）**：SASL/SCRAM-SHA-256（自研 RFC 5802 服务端，未用 rsasl；凭据 env 派生、明文即弃）+ 认证门禁（未认证仅放行 18/17/36，越权/失败断连，58/33/34 语义）+ TLS（rustls 独立口 client+2，通告跟随 listener——账本 57）+ e2e 双档（run_auth.sh librdkafka SCRAM 正/负路径、run_tls.sh SSL 全链）。**矩阵收口（2026-09-18）**：`run_sasl_matrix.sh` 三客户端六项全 PASS（librdkafka SASL_SSL / franz-go SCRAM plain+ssl+负口令 / kafka-clients SASL_SSL 3.9.1 PEM truststore）——矩阵验收 ✅。连带修复账本 61（SCRAM server nonce 字符集非法，Java 严格解析暴露）+ kafka-clients 升 3.9.1（JDK 25 兼容）。**ACL 骨架 ✅（2026-09-18）**：acl.rs（JSON 持久化 + DENY 优先决策面 + 超级用户旁路；BASALT_AUTHORIZER_ENABLED/SUPER_USERS，默认关闭零变化）+ 三 Admin API（29/30/31）+ 数据面闸门（produce/fetch/create/delete/join；fetch v13+ id-only 反查——账本 63）；e2e run_acl.sh 六验收面。**待做**：SCRAM-512/OAUTH、txn-id 闸门、PREFIXED 面推广、host 匹配 | 三客户端 SASL+TLS 矩阵全绿 ✅ | T-M1.5 | [生态 清单](../docs/08-ecosystem.md) [ADR-22](docs/adr-22-security-admin-plane.md) |
| T-M4.2 | 🟨 | 限流与会话 | **quota 一期 ✅（2026-09-18，[ADR-22](docs/adr-22-security-admin-plane.md)）**：逐连接字节率 token bucket（produce 请求字节/fetch 响应字节；负债制 + 持锁休眠串行化——账本 55：pipeline 并发稀释/休眠回补双计两缺陷）；run_quota.sh 限流生效 e2e + testing/bench 吞吐基线（release produce 13.3MB/s acks=all 批延迟主导、consume 1GB/s）。**per-user 覆盖 ✅（2026-09-18）**：`BASALT_QUOTA_USER_BYTES`（user 一维，SASL 认证后套用，回落全局默认；run_quota.sh 阶段二双断言）。**fetch session ✅（KIP-227，2026-09-18）**：fetch_session.rs 会话缓存（epoch 0 建/≥1 增量/-1 关；70/71 响应级错误 + 优雅降级 session_id=0；增量省略空分区）——Java/librdkafka 真会话面全量 e2e 回归绿，状态机单测 ×3。**待做**：fetch 大小 PID 控制器；session HW 变更条目回填（v1 边界，见 fetch_session.rs 模块注） | quota 生效有指标 ✅（e2e 断言速率）；session 复用对响应字节的基准 ⬜ | T-M2.4 | [Redpanda §7](../docs/02-redpanda.md) [ADR-22](docs/adr-22-security-admin-plane.md) |
| T-M4.3 | 🟨 | 分层存储 + 存储模式插件 | **[ADR-21](docs/adr-21-tiered-storage.md) v1 落地（2026-09-18）**：前置阅读 ✅（Arroyo §11 + Morax §6 两路并评——取 CAS+权威记录路线，弃 RDS 权威）；块 a ObjectStore 四原语（create CAS 冲突回读）+ LocalFs/Memory 双实现 ✅；块 b 分层注册表（写序=段对象→段记录→本地回收，孤儿无害+启动期 GC）+ Log::release_sealed（回收不动 log_start）+ 读穿透/FetchSlice 分流/重启恢复 ✅；块 c e2e ✅（tiered 模式 400 条：滚动上传→本地回收→读穿透→重启恢复全链 PASS）。**联调副产修复：账本 53（幂等 last_seq 语义）/54（终态 fence 语义修订，ADR-18 §14）**。对象侧 retention ✅（2026-09-18，BASALT_RETENTION_MS/SWEEP_MS 周期 sweep + DeleteRecords 联动 + run_tiered_retention.sh 四验收面；连带修复账本 62——单节点重启 MetaService 空簇）。S3 适配器实机硬化 ✅（2026-09-19：durad 信任 + SigV4 双模式，run_tiered_s3.sh + 签名读写全链；前置 = durad 运行于 127.0.0.1:19000 信任模式 + 桶 basalt-tiered）。v2 面全清。**v2 per-topic storage-mode ✅（2026-09-18）**：CreateTopics configs `basalt.storage.mode=tiered` 全链（EnsureTopic → ClusterRecord/快照线 → actor 参数化；env 保留为 broker 级缺省）；e2e run_tiered_pertopic.sh 双题分叉验收 | T-M3.2 | [Arroyo §11](../docs/12-arroyo.md) [Morax §6](../docs/16-morax.md) [ADR-21](docs/adr-21-tiered-storage.md) |
