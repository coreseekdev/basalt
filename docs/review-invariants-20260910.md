...
2. memberGen 级僵尸追踪（逐成员记录所属代）留 v0.4——**暂缓的理性**：当前
   成员校验（`m ∈ members`，MUT-C 有审计位判别力）+ 代令牌（`g = assignGen[p]`，
   MUT-E 有判别力）已覆盖"已退成员/旧代成员"两类僵尸；memberGen 只能再覆盖
   "组内代错位"这一 Kafka 本身也不设防的形态（Kafka fencing 即请求代==组代，
   无逐成员代）。除非出现真实需求，加入它属于无判别力增项——不做。
