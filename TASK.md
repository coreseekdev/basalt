...
| T-M3.6 | 🟨 | share groups（KIP-932） | **评估完成（2026-09-18，[纪要](docs/research/2026-09-18-share-groups-evaluation.md)）**：触发条件达成——三客户端支持度实测全就绪（librdkafka 2.15 SHARE_ACKNOWLEDGE 符号面 / kafka-clients 3.9.1 ShareConsumer 早鸟 / franz-go v1.21.6 `ShareGroup`+`Record.Ack()`）；协议面精读（76 与 848 同构、78/79 区间化 ack、coordinator RPC 族不对外宣告）；落地面 = 848 基建复用 + 分区侧区间化 ack 状态机（Morax INT8RANGE[] 对照收敛）；规模 ≈ 4-5 会话，spike 先行（franz-go 验收主面）。**spike ✅（2026-09-19，GO）**：share_group.rs 内存状态机（成员/epoch fence/轮转分配/区间化交付面：acquire-accept-release-reject/30s record lock/交付上限 5）+ 76/78/79 三 handler + 会话注册面（KIP-227 增量语义，spike 实证①）+ fetch 无 member-epoch 校验（实证②）；run_share_spike.sh franz-go 四验收面 PASS。**待做（v1 面转正）**：块 c share-state log 持久化/块 d 多成员 rebalance 与第二消费者/遗忘分区注册删除/长轮询 | T-M3.3 ✅ | KIP-932 |
