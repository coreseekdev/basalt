...
| T-M2.4 | 🟨 | 复制调优 | 按 follower 聚批、落后副本批量追赶+全局限流、共用心跳 RPC。**基线已实测（2026-09-16，benches/throughput_replicated.py）**：3 副本 acks=all produce 120K msg/s（117.4 MB/s）、单条延迟 p50=152ms（恒定 ⇒ follower pull 节拍主导——调优靶子明确）；追赶 P99 影响待测 | 3 副本 acks=all 吞吐基线 ✅ 已度量；调优（pull 节拍/聚批）+ 追赶 P99 基准待做 | T-M2.3 | [Redpanda §3](../docs/02-redpanda.md) |
