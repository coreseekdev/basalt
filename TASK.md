...
| T-M3.6 | 🟨 | share groups（KIP-932） | **评估完成（2026-09-18，[纪要](docs/research/2026-09-18-share-groups-evaluation.md)）**：触发条件达成——三客户端支持度实测全就绪（librdkafka 2.15 SHARE_ACKNOWLEDGE 符号面 / kafka-clients 3.9.1 ShareConsumer 早鸟 / franz-go v1.21.6 `ShareGroup`+`Record.Ack()`）；协议面精读（76 与 848 同构、78/79 区间化 ack、coordinator RPC 族不对外宣告）；落地面 = 848 基建复用 + 分区侧区间化 ack 状态机（Morax INT8RANGE[] 对照收敛）；规模 ≈ 4-5 会话，spike 先行（franz-go 验收主面）。**立项已批准（2026-09-18）——排 T-M4 尾项（fetch session / ACL 骨架 / S3 实机硬化）之后，首块 = spike（块 a 最小面 + franz-go 拉通出 go/no-go）** | T-M3.3 ✅ | KIP-932 |
