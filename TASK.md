...
| T-M3.3 | 🟨 | KIP-848 新消费组协议 | **设计 ✅（2026-09-17 [ADR-19](docs/adr-19-kip848-consumer-groups.md) 草案，待评审）**：ConsumerGroupHeartbeat 单循环、服务端 Range 分配、member-epoch fence、组类型分叉；块 a 状态机+Range 分配器 ✅（consumer_group_tests ×5：java 对拍/fence 三态/幂等续租/退订/describe）；**块 b 协议面 ✅（2026-09-18：68/69 v0 handler+宣告+probe_consumer_group_layouts 字节级回归；fenced 三态映射 82/25 + describe 未知组 69；TopicId 反查 RoutingTable::name_for（名字哈希派生，零新增状态面）+ 分区数快照经 MetaCmd::Lookup 携带）**。**待落地：e2e（c，客户端门槛定案：倾向 franz-go 先行）/ConsumerGroup.tla 扩展+阴性对照（d）**。原待办： 新旧协议混布 rebalance 收敛；客户端（kafka-clients 4.x）跑通 | T-M1.1 | [Kafka §5](../docs/01-apache-kafka.md) |
