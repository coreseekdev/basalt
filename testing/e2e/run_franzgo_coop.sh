#!/usr/bin/env bash
# cooperative-sticky rebalance e2e（T-M3.4 块 c，ADR-20 §4）：
# 起 broker → 跑 testing/franzgo/coop（classic 路径协作协议）→ 收尾。
# 验收面：混布收敛 / 增量 rebalance 无停等 / 双隔离级消费。
set -u
PORT="${1:-9092}"
DATA=$(mktemp -d /tmp/basalt-coop-XXXX)
LOG=$(mktemp /tmp/basalt-coop-log-XXXX)
...
(cd testing/franzgo && go run ./coop)
RC=$?
if [ $RC -ne 0 ]; then
