...
| ㉟ | acks=all 放行竞态：advance_hw 的 min 只算 fresh（isr_lag 窗口内上报）follower——laggard 过期即被静默跳过，HW 越过其 LEO；failover 轮转恰好选中该副本为新 leader 时，divergent-tail healing 截掉多数派已 ack 的尾部消息（failover e2e k-39 实证；控制器 WAL 的 fsync 抖动使 pull 节拍更易跨过 isr_lag 窗口，A/B 实证 pre-WAL 3/3 vs post-WAL 1/3）| 提交面检查与放行条件不一致（C1 只钉了提交时检查，放行侧漏防）| LEO-REPORT/HW-ADV/ACK-RELEASE 探针链（BASALT_LEO_PROBE 门控）+ A/B 二分 | **🚧 已登记未修**——正确修复 = ISR 收缩状态机（收缩为显式动作、恢复需追平重回 ISR、HW=min(ISR LEO) 全程一致），T-M2.2 首项；建议规格化（CommitPipeline.tla 同款：HW/ISR/提交面三者一致性）|
