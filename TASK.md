...
| T-M3.3 | 🟨 | KIP-848 新消费组协议 | **设计 ✅（2026-09-17 [ADR-19](docs/adr-19-kip848-consumer-groups.md) 草案，待评审）**：ConsumerGroupHeartbeat 单循环、服务端 Range 分配、member-epoch fence、组类型分叉；块 a 状态机+Range 分配器 ✅（consumer_group_tests ×6：java 对拍/fence 三态/幂等续租/退订/describe/keepalive）；块 b 协议面 ✅（68/69 v0 handler+宣告+字节级回归；fenced 三态映射 82/25 + describe 未知组 69；TopicId 反查 + 分区数快照经 Lookup）；**块 c 验收面 ✅（2026-09-18：franz-go v1.21.6 三验收面全过——混布收敛/增量 rebalance 无停等（存量成员间隙 201ms）/双隔离级；宣告升 0-1（franz-go should848 硬性 v1）+ codec 补 nullable-struct 原语（账本 51）+ 订阅 keepalive null 语义）**。**待落地：ConsumerGroup.tla 扩展+阴性对照（d）**。客户端门槛已定案：franz-go 先行，java 4.x 单列 | T-M1.1 | [Kafka §5](../docs/01-apache-kafka.md) |
