...
## franz-go 直连面基线（2026-09-18 修订：账本 58 修复后，T-S5 配额基准配套）

`testing/bench/run_bench.sh`（release 二进制 + franz-go 直连：无压缩、手动
分区、MaxBufferedRecords=1M、acks=all 默认、4 分区、200k×1KB、tmpfs）：

| 指标 | 修复前（账本 58） | 修复后 | 说明 |
|---|---|---|---|
| Produce | 13.3 MB/s（8.0k msg/s） | **1702.9 MB/s（170 万 msg/s）** | 5s 成簇请求 + PID reload 级联 → produce 按连接序应用后零 OOOSN |
| Consume | 1022 MB/s | 1058 MB/s | 读路径本就无阻塞，未受影响 |
| 计数一致性 | 200118/200000（118 条重复） | **200000/200000 零重复** | 同一根因：OOOSN 级联的回卷重发 |

**教训**：修复前曾把重复误判为「PID 未持久化」（非事务 PID per-session 本
就是 Kafka 语义，方向作废）；13.3MB/s 也曾被记为「acks=all 批延迟主导」
——两结论皆错。真根因 = produce 处理序乱序（账本 58），双重表现互为掩护。
