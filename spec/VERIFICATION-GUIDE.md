...
| P1-2 | **Crash 不丢未持久化尾巴**：模型里 append 即持久，Crash 全量保留 `log[n]`；而 sim_disk 故障模型（VERIFICATION.md §3）声明"已 sync 存活、未 sync 消失"，且把 TLA+ 环境动作列为该模型的消费方。现状是文档与模型不符 | 实现 ack 前不 fsync 的 bug 规约层抓不到 | 加 `synced[n] ≤ Len(log[n])` 边界：Persist 动作推进 synced、Crash 截断 log 到 synced、`CommitAdvance` 的 `SameVals` 改为比较 synced 前缀。若改动后不变式仍绿，则同时证明了"commit ⇒ 多数派已持久"这条实现纪律的必要性 |
