...
async fn async_main(cfg: Config) {
    ...
    // share 组状态监督（B4）：__share_group_state ensure → leader 时重放 +
    // 注入事件下沉通道。事件只在本节点为该分区 leader 时持久化（多节点
    // share 协调 = B5 面）；重放安装游标/归档（acquired 锁为瞬态）。
    {
        let itx = meta_tx.clone();
        let routes_rx_s = routes_rx.clone();
        let self_node = cfg.node_id;
        let rf = cfg.default_rf;
        tokio::spawn(async move {
            for _ in 0..20 {
            ...
            let (txr, rxr) = tokio::sync::oneshot::channel();
            let _ = itx.send(meta::MetaCmd::EnsureTopic {
                name: "__share_group_state".to_string(),
                partitions: 1,
                rf,
                tiered: false,
                reply: txr,
