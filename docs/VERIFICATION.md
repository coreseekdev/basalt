...
| C7' | **opfuzz 首轮发现并修复 2 个真实缺陷**：① SimDisk::len 只返回 pending（crash 后 seg.bytes=0，Log::open 簿记/roll 空段判定失真）；② truncate_to 盲设 next_offset=offset 与批边界错位（重启恢复按位置重排 offset，客户端可见漂移）。**开放发现 1 个**：delete/truncate 与 roll/crash 交互后重开丢数据（seed=1 clean 序列已捕获） | L1 | cargo test opfuzz（2 档 60 种子，#[ignore] WIP） | 🚧 2 修复 + 1 开放（2026-09-09） | `cargo test -p basalt-storage --test opfuzz -- --ignored` |
