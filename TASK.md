...
| T-M3.3.1 | ✅ | 848 生命周期硬化 | **[研究对照](docs/research/2026-09-18-reference-comparison.md) 行动清单全量落地（2026-09-18）**：P0 session timeout（成员滑动 timer + deadline sweep，崩溃分区泄漏封口）+ 撤销确认闭环（owned 跟踪 + pending_release 门控，双 owner 消除）；P1 差分下发（未变→Assignment=null + full-request 判定）+ previous-epoch 容错/epoch-0 rejoin（应答丢失恢复）；P2 Describe 三值状态（含 Reconciling）/interval 节流/group max size（81）/SubscribedTopicRegex 同步解析（names∪resolved）/静态成员（-2 paused + InstanceId）/rebalance timeout（撤销宽限）；coordinator 测试 8→14；**franz-go 848 e2e 全链零回归** | T-M3.3 ✅ | kafka trunk 对照 |
