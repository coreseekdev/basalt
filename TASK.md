...
| T-M3.3.1 | 🟨 | 848 生命周期硬化（[研究对照](docs/research/2026-09-18-reference-comparison.md) 行动清单） | **P0**：session timeout（成员 timer+心跳续期+超时注销重算）；撤销确认闭环（owned 跟踪 + pending-release 门控，双 owner 消除）。**P1**：差分下发（未变→Assignment=null + full-request 判定）；previous-epoch 容错 + epoch-0 rejoin。**P2**：Describe 三值状态（Empty/Reconciling/Stable）；assignment interval 节流；group max size；SubscribedTopicRegex（同步解析变体）；static membership（-2 临时离开 + InstanceId 顶替）；rebalance timeout | T-M3.3 ✅ | kafka trunk 对照（docs/research/2026-09-18） |
