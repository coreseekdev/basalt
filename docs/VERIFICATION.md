...
| ㊼ | 阴性对照 cfg 失真：a434850 重做 norecon cfg 时把 `Reconciliation` 写回 TRUE——留下的唯一突变 FrozenFace=FALSE 正是提交说明自载「被 reconciliation 掩盖（符合设计）」的恒绿突变体，阴性对照退化为结构性必绿（406,978 态全绿）；判别力门禁（交接文档 §5 基线）名存实亡 | 门禁配置回归（阴性对照恒真——「检查器自身要活着」纪律的 cfg 层违例）| 接手基线重跑（§5 预期一绿一红，实测四绿）+ 探针实证含对照组：现 cfg 恒绿（406,978 态）；恢复 Reconciliation=FALSE 单突变即被 InvLeaderServingHasAcked 检出（1,524 态 / 双突变 1,641 态）| cfg 修复为账本 ㉟ 记载的单突变体（Reconciliation=FALSE + FrozenFace=TRUE，MaxEntries=3，重跑四门禁一绿一红两绿：39,802/1,470 检出/152/39,802）；CI 反转断言（`if make norecon; then error`）本应在线上拦截——a434850 推送后未核查 Actions 运行即为漏网窗口，cfg 类改动推送后必须回看 CI；突变 cfg 三方一致纪律：Makefile 注释 ↔ 账本 ㉟ ↔ cfg 突变参数逐项核对 |
